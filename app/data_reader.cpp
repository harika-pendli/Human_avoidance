...
void DataReader::setImagepath(const std::string &path) { img_path = path; }
void DataReader::setVideopath(const std::string &path) { video_path = path; }

std::string DataReader::getInput(cv::CommandLineParser parser,
...
int DataReader::getOutputWidth() { return outputWidth; }

int DataReader::getOutputHeight() { return outputHeight; }

void DataReader::setOutputHeight(int height) { outputHeight = height; }

void DataReader::setOutputWidth(int width) { outputWidth = width; }
...
cv::VideoCapture DataReader::videoProcessor(char rw, cv::Mat frame,
                                            cv::VideoWriter out) {
  if (rw == 'r') {
